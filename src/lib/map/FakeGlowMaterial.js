...
class FakeGlowMaterial extends ShaderMaterial {
     ...
     * @param {import('three').Side} [parameters.side=THREE.FrontSide] - The rendering side. Use `THREE.FrontSide`, `THREE.BackSide`, or `THREE.DoubleSide`.
    ...
    `;
      ...
      void main()
      {
        ...
        fakeGlow *= falloff;
        float noise = dither(gl_FragCoord.xy) - 0.5;
        float alpha = clamp(fakeGlow + noise * 0.02, 0.0, opacity);
      ...
      `;
        ...
        this.side =
            parameters.side !== undefined ? parameters.side : DoubleSide;
    }
}
